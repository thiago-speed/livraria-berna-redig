...
def listar_livros() -> list[dict[str, Any]]:
    with connectbd() as bd:
        rows = bd.execute("SELECT id, titulo, autor, ano_publicacao, preco FROM livros ORDER BY id").fetchall()
...
def remover_livro(id_livro: int | str) -> bool:
    ...
    with connectbd() as bd:
        cur = bd.execute("DELETE FROM livros WHERE id = ?", (id_ok,))
        bd.commit()
        return cur.rowcount > 0
